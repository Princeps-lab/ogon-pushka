...
const ProductItem = ({product, deleteProduct, addFavor}) => {
...
const Basket = () => {
  ...
  const products = store.products.map(product => <ProductItem
    deleteProduct={store.deleteProduct} addFavor={store.addFavor} key={product.id} product={product}/>);
